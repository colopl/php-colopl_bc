--TEST--
Check for \Colopl\ColoplBc\Php74\lt() function. (Upstream: comparing different variables (less than)) @ 32-bit (colopl_bc.php74.compare_mode=3)
--EXTENSIONS--
colopl_bc
--INI--
date.timezone=UTC
...
foreach($a as $var) {
    for ($i = 0; $i < $var_cnt; $i++) {
        my_dump($var);
        echo \Colopl\ColoplBc\Php74\lt($var, $a[$i]) ? " < " : " >= ";
        my_dump($a[$i]);
        echo "\n";
    }
}
...
Deprecated: Colopl\ColoplBc\Php74\lt(): Incompatible compare detected in %s on line %d
 >= int(0)
