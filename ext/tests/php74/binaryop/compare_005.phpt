--TEST--
Check for \Colopl\ColoplBc\Php74\gte() function. (Upstream: comparing different variables (greater or equal than)) @ 32-bit (colopl_bc.php74.compare_mode=3)
--EXTENSIONS--
colopl_bc
--INI--
date.timezone=UTC
...
--SKIPIF--
<?php if (PHP_VERSION_ID < 80000) die("skip PHP 8 compatibility diagnostics only"); if (PHP_INT_SIZE != 4) die("skip this test is for 32bit platform only"); ?>
--FILE--
<?php
...
foreach($a as $var) {
    for ($i = 0; $i < $var_cnt; $i++) {
        my_dump($var);
        echo \Colopl\ColoplBc\Php74\gte($var, $a[$i]) ? " >= " : " < ";
        my_dump($a[$i]);
        echo "\n";
    }
}
...
Deprecated: Colopl\ColoplBc\Php74\gte(): Incompatible compare detected in %s on line %d
 >= int(0)
...
int(-2147483648)Incompatible compare detected in %s on line %d
...
string(11) "-2147483648" < object(stdClass)#%d (0) {}
string(11) "-2147483648" < object(stdClass)#%d (0) {}
string(11) "-2147483648" < object(test)#%d (0) {}
